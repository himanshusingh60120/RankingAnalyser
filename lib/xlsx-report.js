...
export function buildHreflangWorkbook(r) {
  ...
  const perWeekCols = 5; // URLs with Data, Clicks, Impressions, CTR, Avg Position

  // =========================== SUMMARY ===========================
  const cols = 4 + weeks.length * perWeekCols;
  ...
  ws.mergeCells(h1, 3, h2, 3); headCell(ws.getCell(h1, 3), "Indexed URLs");
  ws.mergeCells(h1, 4, h2, 4); headCell(ws.getCell(h1, 4), "Unindexed URLs");
  weeks.forEach((w, i) => {
    const c0 = 5 + i * perWeekCols;
    ws.mergeCells(h1, c0, h1, c0 + perWeekCols - 1);
    headCell(ws.getCell(h1, c0), wkTitle(w.label) + "\n" + dateSpan(w.startDate, w.endDate));
    ["URLs with Data", "Clicks", "Impressions", "CTR (%)", "Avg Position"]
  ...
  (r.sitemapReports || []).forEach((sr, si) => {
    ...
    dataCell(ws.getCell(row, 3), sr.indexedUrls, NUM, { band });
    dataCell(ws.getCell(row, 4), sr.unindexedUrls, NUM, { band });
    sr.weeks.forEach((wt, wi) => {
      const c0 = 5 + wi * perWeekCols;
  ...
  fx(ws.getCell(row, 3), `SUM(C${firstDataRow}:C${lastDataRow})`,
    (r.sitemapReports || []).reduce((n, s) => n + (s.indexedUrls || 0), 0), NUM, { bold: true });
  fx(ws.getCell(row, 4), `SUM(D${firstDataRow}:D${lastDataRow})`,
    (r.sitemapReports || []).reduce((n, s) => n + (s.unindexedUrls || 0), 0), NUM, { bold: true });
  ...
  if (weeks.length) {
    ...
    row++;
    const heads = ["Rank", "Language", "URLs with Data", "Clicks", "Impressions", "CTR (%)", "Avg Position"];
  ...
  for (let c = 5; c <= cols; c++) ws.getColumn(c).width = 12;
  ...
  for (const sr of r.sitemapReports || []) {
    ...
    let lr = uh2 + 1;
    // every checked URL for this language: indexed first, then unindexed
    // (urlRows arrives already sorted from the endpoint)
    const langUrls = urls.filter((u) => (u.lang || "") === (sr.lang && sr.lang.code));
    langUrls.forEach((u, i) => {
      const band = i % 2 === 1;
      const indexed = u.indexed !== false;
      dataCell(lws.getCell(lr, 1), u.url, null, { left: true, band });
      dataCell(lws.getCell(lr, 2), indexed ? "Indexed" : "Unindexed", null, { left: true, band });
      weeks.forEach((w, wi) => {
        const c0 = 3 + wi * 4;
        const pw = u.weeks[wi] || {};
