...
export function auditPage(x, keyword, contentText = "") {
  ...
  // H1
  // H1 — document-wide list (hero H1s often sit outside the content container)
  const h1s = (x.h1List && x.h1List.length)
    ? x.h1List
    : x.headingTree.filter(([lvl]) => lvl === 1).map(([, t]) => t);
  if (h1s.length === 0) add("critical", "Headings", "No H1 found.");
  else if (h1s.length > 1) add("warning", "Headings", `Multiple H1s (${h1s.length}). Prefer one.`, h1s.join(" | "));
  else {
    add("good", "Headings", "Single H1 present.", h1s[0]);
    if (kw && !h1s[0].toLowerCase().includes(kw)) add("warning", "Headings", "Keyword not in H1.");
  }
