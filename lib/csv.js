...
export function parseTitleUrlCsv(text) {
  ...
  // Layout detection: usually Title=col A, URL=col B — but also accept a
  // URL-only CSV (or URL in col A) by checking which column holds the URLs.
  const probe = grid.slice(0, 5);
  const urlHitsB = probe.filter((r) => isUrl(r[1])).length;
  const urlHitsA = probe.filter((r) => isUrl(r[0])).length;
  const urlCol = urlHitsA > urlHitsB ? 0 : 1;
  const titleCol = urlCol === 0 ? null : 0;

  let start = 0;
  let skippedHeader = false;
  // header detection: first row's URL column isn't a URL -> header line
  if (!isUrl(grid[0][urlCol])) { start = 1; skippedHeader = true; }

  const rows = [];
  for (let i = start; i < grid.length; i++) {
    const cells = grid[i];
    const title = titleCol == null ? "" : (cells[titleCol] || "").trim();
