...
function stripChrome($, $main) {
  ...
  $main.find("div, section, ul, ol, aside, nav").each((_, el) => {
    ...
    const ident = identOf($, el);
    if (!ident) return;
    const strong = STRONG_CHROME.test(ident);
    const weak = CHROME_HINTS.test(ident) && !MAIN_HINTS.test(ident);
    if (!strong && !weak) return;
    // SAFETY: a widget-sounding class can also wrap the real article (e.g. a
    // tabbed report panel). Removing it would wipe the content — exactly the
    // failure where a page reports 0 in-content links. So only strip elements
    // that actually behave like widgets: link-dominated or text-poor. A
    // text-rich, low-link block is treated as content and kept.
    const txt = $el.text().trim();
    const linkTxt = $el.find("a").text().trim();
    const linkShare = txt.length ? linkTxt.length / txt.length : 1;
    if (txt.length > 400 && linkShare < 0.5) return; // real content — keep it
    $(el).remove();
  });
}
