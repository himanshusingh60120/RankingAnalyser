 ...
 */
/**
 * Does a page URL belong to a given language code? Compares the URL's own
 * detected language (from its path) to the group code. "x-default" matches
 * root URLs that carry no language prefix (the default/English pages).
 * @param {string} url
 * @param {string} code  language code of the group, e.g. "fr" or "x-default"
 */
export function urlMatchesLang(url, code) {
  const c = detectLang(url).code;
  if (code === "x-default") return c === "x-default";
  // exact ("fr" === "fr") or regional under the same base ("pt-br" vs "pt")
  return c === code || c.split("-")[0] === code.split("-")[0];
}
...
export function detectLang(url) {
  ...
  return { code: "x-default", label: "English" };
}
