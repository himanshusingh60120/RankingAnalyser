...
export async function findCompetitorUrls(keyword, contentType = "report") {
  ...
  for (const site of COMPETITOR_SITES) {
    ...
    if (!url) {
      url = await viaOnSiteSearch(keyword, site, isBlog);
      if (url) method = isBlog ? "on-site-search(blog)" : "on-site-search";
    }
...
async function viaSearchProvider(keyword, site, isBlog = false) {
  ...
  const q = isBlog ? `${keyword} blog OR insights OR article` : `${keyword} market report`;
  // When matching blogs, reject competitor *report* URLs (we want their articles).
  const accept = (link) => {
    if (!link || !link.includes(site)) return false;
    if (isBlog) return !/\/reports?\/|-market-?\d*\/?$|\/report\//i.test(link);
    return true;
  };
  ...
  if (gKey && gCx) {
    try {
      ...
      if (r.ok) {
        ...
        const item = (data.items || []).find((it) => accept(it.link));
  ...
  if (bKey) {
    try {
      ...
      if (r.ok) {
        const data = await r.json();
        const item = (data.webPages?.value || []).find((it) => accept(it.url));
